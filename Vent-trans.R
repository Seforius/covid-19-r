...
v <- function(T, N) {
  return(c(replicate((T+1)*N^2,1e-04),replicate((T+1)*N,2), replicate((T+1)*2*N,0)))
  }
...
mat <-function(T, N) {
  ...
  l <- zer(N*T,(T+1)*N)
  ii<- cbind(bdiag(ee, N), zer(N, T*N^2))   
...
#print(B)
B <- c(req_vent, rep(0, ((2*T_periods+1)*N_nodes)), init_resp) 
print(B)

#Lastly fill in the constraint directions
constraints_direction  <- c("=")
for(i in 2:(((T_periods+1))*N_nodes))
{
